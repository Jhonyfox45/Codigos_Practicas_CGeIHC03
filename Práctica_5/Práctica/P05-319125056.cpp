...
//Modelos para el holocron
Model Base_holocron_M;
Model Esquina_superior_M;
Model Esquina_inferior_M;
//Modelos para nuestro satelite
Model Base_satelite_M;
Model Panel_derecha1_M;
Model Panel_derecha2_M;
Model Panel_izquierda1_M;
Model Panel_izquierda2_M;
...
const float movX_Rover = -30.0f;
const float movY_Rover = 1.5f;
const float movZ_Rover = -5.0f;

const float movX_Holocron = 0.0f;
const float movY_Holocron = 6.0f;
const float movZ_Holocron = -5.0f;

const float movX_Satelite = 40.0f;
const float movY_Satelite = 8.0f;
const float movZ_Satelite = -5.0f;
...
int main()
{
	...
	//Cargamos los modelos para el holocron
	Base_holocron_M = Model();
	Base_holocron_M.LoadModel("Models/Base_holocron.obj");
	Esquina_superior_M = Model();
	Esquina_superior_M.LoadModel("Models/Esquina_superior.obj");
	Esquina_inferior_M = Model();
	Esquina_inferior_M.LoadModel("Models/Esquina_inferior.obj");

	//Cargamos los modelos para nuestro satelite
	Base_satelite_M = Model();
	Base_satelite_M.LoadModel("Models/Base_satelite.obj");
	Panel_derecha1_M = Model();
	Panel_derecha1_M.LoadModel("Models/Panel_derecha1.obj");
	Panel_derecha2_M = Model();
	Panel_derecha2_M.LoadModel("Models/Panel_derecha2.obj");
	Panel_izquierda1_M = Model();
	Panel_izquierda1_M.LoadModel("Models/Panel_izquierda1.obj");
	Panel_izquierda2_M = Model();
	Panel_izquierda2_M.LoadModel("Models/Panel_izquierda2.obj");
	...
	while (!mainWindow.getShouldClose())
	{
		...
		//A PARTIR DE AQUÍ
		//MODELAMOS NUESTRO HOLOCRON
		//Renderizamos la base del holocron
		model = glm::mat4(1.0);
		model = glm::translate(model, glm::vec3(0.0f + movX_Holocron, 0.0f + movY_Holocron, 0.0f + movZ_Holocron));
		glUniformMatrix4fv(uniformModel, 1, GL_FALSE, glm::value_ptr(model));
		color = glm::vec3(1.0f, 0.1f, 0.1f); //Rojo brillante
		glUniform3fv(uniformColor, 1, glm::value_ptr(color));
		Base_holocron_M.RenderModel();
		modelaux = model;
		modelaux2 = model;
		
		//Renderizamos las esquinas superiores
		//Esquina superior 1
		model = modelaux2;
		model = glm::translate(model, glm::vec3( 3.85f, 3.9f, -3.85f));
		model = glm::rotate(model, glm::radians(mainWindow.getarticulacion1H()), glm::vec3(0.5736f, 0.5792f, -0.5792f)); //Rotación diagonal, calculo del vector
		modelaux = model;
		glUniformMatrix4fv(uniformModel, 1, GL_FALSE, glm::value_ptr(model));
		color = glm::vec3(0.1f, 0.8f, 0.1f); //Verde brillante
		glUniform3fv(uniformColor, 1, glm::value_ptr(color));
		Esquina_superior_M.RenderModel();
		//Esquina superior 2
		model = modelaux2;
		model = glm::translate(model, glm::vec3(-3.85f, 3.9f, -3.85f));
		model = glm::rotate(model, glm::radians(90.0f), glm::vec3(0.0f, 1.0f, 0.0f));
		model = glm::rotate(model, glm::radians(mainWindow.getarticulacion2H()), glm::vec3(0.5736f, 0.5792f, -0.5792f)); //Rotación diagonal, calculo del vector
		modelaux = model;
		glUniformMatrix4fv(uniformModel, 1, GL_FALSE, glm::value_ptr(model));
		color = glm::vec3(0.1f, 0.3f, 0.9f);// Azul vibrante
		glUniform3fv(uniformColor, 1, glm::value_ptr(color));
		...
		model = glm::rotate(model, glm::radians(mainWindow.getarticulacion3H()), glm::vec3(0.5736f, 0.5792f, -0.5792f)); //Rotación diagonal, calculo del vector
		modelaux = model;
		glUniformMatrix4fv(uniformModel, 1, GL_FALSE, glm::value_ptr(model));
		color = glm::vec3(1.0f, 0.9f, 0.1f);// Amarillo puro
		glUniform3fv(uniformColor, 1, glm::value_ptr(color));
		...
		model = glm::rotate(model, glm::radians(mainWindow.getarticulacion4H()), glm::vec3(0.5736f, 0.5792f, -0.5792f)); //Rotación diagonal, calculo del vector
		modelaux = model;
		glUniformMatrix4fv(uniformModel, 1, GL_FALSE, glm::value_ptr(model));
		color = glm::vec3(0.9f, 0.1f, 0.6f);// Magenta
		glUniform3fv(uniformColor, 1, glm::value_ptr(color));
		...
		//Renderizamos las esquinas inferiores
		//Esquina inferior 1
		model = modelaux2;
		model = glm::translate(model, glm::vec3(3.85f, -3.9f, -3.85f));
		model = glm::rotate(model, glm::radians(mainWindow.getarticulacion5H()), glm::vec3(0.5736f, -0.5792f, -0.5792f)); //Rotación diagonal, calculo del vector
		modelaux = model;
		glUniformMatrix4fv(uniformModel, 1, GL_FALSE, glm::value_ptr(model));
		color = glm::vec3(0.0f, 0.8f, 0.8f); //Cian
		glUniform3fv(uniformColor, 1, glm::value_ptr(color));
		Esquina_inferior_M.RenderModel();
		//Esquina inferior 2
		model = modelaux2;
		model = glm::translate(model, glm::vec3(-3.85f, -3.9f, -3.85f));
		model = glm::rotate(model, glm::radians(90.0f), glm::vec3(0.0f, 1.0f, 0.0f));
		model = glm::rotate(model, glm::radians(mainWindow.getarticulacion6H()), glm::vec3(0.5736f, -0.5792f, -0.5792f)); //Rotación diagonal, calculo del vector
		modelaux = model;
		glUniformMatrix4fv(uniformModel, 1, GL_FALSE, glm::value_ptr(model));
		color = glm::vec3(1.0f, 0.5f, 0.0f); //Naranja
		glUniform3fv(uniformColor, 1, glm::value_ptr(color));
		...
		model = glm::translate(model, glm::vec3(-3.85f, -3.9f, 3.85f));
		model = glm::rotate(model, glm::radians(180.0f), glm::vec3(0.0f, 1.0f, 0.0f));
		model = glm::rotate(model, glm::radians(mainWindow.getarticulacion7H()), glm::vec3(0.5736f, -0.5792f, -0.5792f)); //Rotación diagonal, calculo del vector
		modelaux = model;
		glUniformMatrix4fv(uniformModel, 1, GL_FALSE, glm::value_ptr(model));
		color = glm::vec3(0.5f, 0.0f, 0.8f); //Morado
		glUniform3fv(uniformColor, 1, glm::value_ptr(color));
		...
		model = glm::translate(model, glm::vec3(3.85f, -3.9f, 3.85f));
		model = glm::rotate(model, glm::radians(270.0f), glm::vec3(0.0f, 1.0f, 0.0f));
		model = glm::rotate(model, glm::radians(mainWindow.getarticulacion8H()), glm::vec3(0.5736f, -0.5792f, -0.5792f)); //Rotación diagonal, calculo del vector
		modelaux = model;
		glUniformMatrix4fv(uniformModel, 1, GL_FALSE, glm::value_ptr(model));
		color = glm::vec3(0.6f, 1.0f, 0.1f); //Verde lima
		glUniform3fv(uniformColor, 1, glm::value_ptr(color));
		Esquina_inferior_M.RenderModel();
		//CON ESTO TERMINAMOS EL HOLOCRON

		//A PARTIR DE AQUÍ
		//MODELAMOS NUESTRO SATELITE
		//Renderizamos la base del satelite
		model = glm::mat4(1.0);
		//Añadimos las variables de movimiento en la traslación de la base
		model = glm::translate(model, glm::vec3(mainWindow.getvarMovX() + movX_Satelite,mainWindow.getvarMovY() + movY_Satelite,mainWindow.getvarMovZ() + movZ_Satelite));
		glUniformMatrix4fv(uniformModel, 1, GL_FALSE, glm::value_ptr(model));
		color = glm::vec3(1.0f, 0.1f, 0.1f); //Rojo brillante
		glUniform3fv(uniformColor, 1, glm::value_ptr(color));
		Base_satelite_M.RenderModel();
		modelaux = model;
		modelaux2 = model;

		//Ahora construimos la parte de los paneles de la derecha
		//Panel derecha 1
		model = modelaux2;
		model = glm::translate(model, glm::vec3( 11.5f, 0.0f, 0.0f));
		model = glm::rotate(model, glm::radians(mainWindow.getarticulacionsonda1()), glm::vec3(0.0f, 0.5299f, 0.8480f)); //Otro eje diagonal que debemos calcular
		modelaux = model;
		glUniformMatrix4fv(uniformModel, 1, GL_FALSE, glm::value_ptr(model));
		color = glm::vec3(0.1f, 0.8f, 0.1f); //Verde brillante
		glUniform3fv(uniformColor, 1, glm::value_ptr(color));
		Panel_derecha1_M.RenderModel();
		//Panel derecha 2
		model = modelaux;
		model = glm::translate(model, glm::vec3(8.5f, 0.0f, 0.0f));
		model = glm::rotate(model, glm::radians(-mainWindow.getarticulacionsonda2()), glm::vec3(0.0f, 0.5299f, 0.8480f)); //Otro eje diagonal que debemos calcular
		modelaux = model;
		glUniformMatrix4fv(uniformModel, 1, GL_FALSE, glm::value_ptr(model));
		color = glm::vec3(0.1f, 0.3f, 0.9f);// Azul vibrante
		glUniform3fv(uniformColor, 1, glm::value_ptr(color));
		Panel_derecha2_M.RenderModel();

		//Ahora construimos la parte de los paneles de la izquierda
		//Panel izquierda 1
		model = modelaux2;
		model = glm::translate(model, glm::vec3(-11.5f, 0.0f, 0.0f));
		model = glm::rotate(model, glm::radians(-mainWindow.getarticulacionsonda3()), glm::vec3(0.0f, 0.5299f, 0.8480f)); //Otro eje diagonal que debemos calcular
		modelaux = model;
		glUniformMatrix4fv(uniformModel, 1, GL_FALSE, glm::value_ptr(model));
		color = glm::vec3(1.0f, 0.9f, 0.1f);// Amarillo puro
		glUniform3fv(uniformColor, 1, glm::value_ptr(color));
		Panel_izquierda1_M.RenderModel();
		//Panel izquierda 2
		model = modelaux;
		model = glm::translate(model, glm::vec3(-8.5f, 0.0f, 0.0f));
		model = glm::rotate(model, glm::radians(mainWindow.getarticulacionsonda4()), glm::vec3(0.0f, 0.5299f, 0.8480f)); //Otro eje diagonal que debemos calcular
		modelaux = model;
		glUniformMatrix4fv(uniformModel, 1, GL_FALSE, glm::value_ptr(model));
		color = glm::vec3(0.9f, 0.1f, 0.6f);// Magenta
		glUniform3fv(uniformColor, 1, glm::value_ptr(color));
		Panel_izquierda2_M.RenderModel();
